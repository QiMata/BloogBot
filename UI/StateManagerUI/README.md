...
A WPF desktop application that provides a graphical user interface for monitoring and managing World of Warcraft bot instances in the WWoW ecosystem.
...
The application displays real-time bot status including character name and level, current health/mana percentages, position coordinates, current activity/state, and connection status. It integrates with StateManager service for backend communication and provides visual feedback for bot operations and server health.

Key capabilities include multi-character management with tabular display, real-time activity monitoring, server status dashboard for MaNGOS realm and world server connectivity, and personality configuration using the Big Five trait model with decimal precision controls.
...
+-- MainWindow.xaml.cs
+-- Views/
|   +-- StateManagerViewModel.cs  # Main view model (MVVM pattern)
+-- Handlers/
|   +-- CommandHandler.cs         # ICommand implementation
+-- Converters/
|   +-- ValueConverter.cs         # Generic value converter base
|   +-- GreaterThanZeroToBooleanConverter.cs
+-- BasicLogger.cs                # Simple logging utility
+-- AssemblyInfo.cs
```

### Core Components

**MainWindow**: Primary WPF window hosting all UI elements with responsive 13x12 grid layout

**StateManagerViewModel**: MVVM pattern implementation with data binding and observable collections

**CommandHandler**: Command pattern implementation for decoupled user action handling

**Value Converters**: Data transformation for UI binding scenarios
...
- **Automatic Polling**: Periodic status checks with configurable 10-second intervals
...
- **Character Selection**: Individual character focus for detailed management with previous/next navigation
- **Account Information**: Display of character names, races, classes, levels, continent, and map
...
- **Value Precision**: Decimal precision for fine-tuned personality control using sliders and numeric input boxes
...
- **Connection Controls**: Connect, disconnect, and local state management buttons
...
1. Ensure StateManager service is running on port 8088
2. Launch StateManagerUI.exe or run via `dotnet run`
...
- **Memory Usage**: Minimal impact with efficient WPF data binding (~50MB base footprint)
- **Network Overhead**: Low bandwidth usage with periodic polling (<1KB/s per character)
...
- See [StateManager README](../../Services/StateManager/README.md) for backend service
- See [BotCommLayer README](../../Exports/BotCommLayer/README.md) for communication layer
- See [UI Layer README](../README.md) for UI architecture overview
