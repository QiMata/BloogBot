...
## Session Handoff
### 2026-04-25 (Shodan dual-client/movement parity config observation)
...
- Last delta:
  - `DualClientParityTests` and `MovementParityTests` now run against `Economy.config.json` with `ECONBG1` and `ECONFG1` as parity action targets and SHODAN as director.
  - Shared Orgrimmar snapshot staging and movement route-start staging moved into live-validation fixture helpers; no WoWStateManager runtime or config change was required.
- Pass result: `Dual-client/movement migration shape green; live validation passed overall with 10 passes and 7 tracked skips`
- Validation/tests run:
  ...
  - `dotnet test ... --filter "FullyQualifiedName~DualClientParityTests|FullyQualifiedName~MovementParityTests" --logger "trx;LogFileName=dual_movement_parity_shodan_final2.trx"` -> `passed overall (10 passed, 7 skipped)`
...
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `TaxiTests`, `TaxiTransportParityTests`, and `TransportTests` now run against `Economy.config.json` with `ECONBG1` as the BG taxi/transport action target, `ECONFG1` idle or parity-active by test, and SHODAN as director.
  - Taxi readiness and transport coordinate staging moved into `LiveBotFixture` helpers; no WoWStateManager runtime or config change was required.
- Pass result: `Transport/taxi migration shape green; live validation passed overall with 8 passes and 5 tracked skips`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~TaxiTests|FullyQualifiedName~TaxiTransportParityTests|FullyQualifiedName~TransportTests" --logger "trx;LogFileName=transport_taxi_shodan_final.trx"` -> `passed overall (8 passed, 5 skipped)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
...
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `BattlegroundQueueTests` now runs against `Economy.config.json` with `ECONBG1` as the BG WSG queue action target, `ECONFG1` launched idle for topology parity, and SHODAN as director.
  - WSG minimum-level setup and Orgrimmar battlemaster staging moved into `LiveBotFixture` helpers; no WoWStateManager runtime change was required.
- Pass result: `BattlegroundQueue migration shape green; live WSG queue validation passed 1/1`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~BattlegroundQueueTests" --logger "trx;LogFileName=battleground_queue_shodan.trx"` -> `passed (1/1)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
...
### 2026-04-25 (Shodan BgInteraction config observation)
- Last updated: 2026-04-25
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `BgInteractionTests` now runs against `Economy.config.json` with `ECONBG1` as the BG economy/NPC smoke action target, `ECONFG1` launched idle for topology parity, and SHODAN as director.
  - Bank, auction-house, mailbox, mail-money, coinage, and flight-master setup moved into `LiveBotFixture` helpers; no WoWStateManager runtime change was required.
- Pass result: `BgInteraction migration shape green; live validation passed overall with 3 BG passes and 2 tracked skips`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~BgInteractionTests" --logger "trx;LogFileName=bg_interaction_shodan.trx"` -> `passed overall (3 passed, 2 skipped)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
...
- Active task: none - this slice reused the existing Shodan loot roster and did not change WoWStateManager code or config.
- Last delta:
  - `DeathCorpseRunTests` now runs against `Loot.config.json` with `LOOTBG1` as the corpse-run action target, `LOOTFG1` launched for topology parity, and SHODAN as director.
  - Razor Hill corpse staging and cleanup moved into `LiveBotFixture` helpers; no WoWStateManager runtime change was required.
- Pass result: `DeathCorpseRun migration shape green; live corpse-run validation passed overall with 1 BG pass and 1 FG opt-in skip`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~DeathCorpseRunTests" --logger "trx;LogFileName=death_corpse_run_shodan.trx"` -> `passed overall (1 passed, 1 skipped)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
...
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `CornerNavigationTests` and `TileBoundaryCrossingTests` now run against `Economy.config.json` with `ECONBG1` as the navigation action target, `ECONFG1` idle for topology parity, and SHODAN as director.
  - Navigation coordinate staging moved into `LiveBotFixture` helpers; no WoWStateManager runtime change was required.
- Pass result: `Corner/tile navigation migration shape green; live navigation validation passed 6/6`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~CornerNavigationTests|FullyQualifiedName~TileBoundaryCrossingTests" --logger "trx;LogFileName=corner_tile_navigation_shodan.trx"` -> `passed (6/6)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
...
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `MapTransitionTests` now runs against `Economy.config.json` with `ECONBG1` as the Deeprun Tram bounce action target, `ECONFG1` idle for topology parity, and SHODAN as director.
  - Ironforge tram staging, rejected Deeprun Tram transition setup, and cleanup moved into `LiveBotFixture` helpers; no WoWStateManager runtime change was required.
- Pass result: `MapTransition migration shape green; live Deeprun Tram bounce validation passed 1/1`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~MapTransitionTests" --logger "trx;LogFileName=map_transition_shodan.trx"` -> `passed (1/1)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
...
- Last delta:
  - Added `NpcInteraction.config.json` with `NPCBG1` Background Orc Hunter, `NPCFG1` Foreground Orc Rogue, and SHODAN as Background Gnome Mage director.
  - The roster keeps both BotRunner action targets online for vendor, flight-master, and object-manager NPC checks while preserving a hunter target for the trainer path.
- Pass result: `NPC interaction migration shape green; live NPC validation passed 3 and skipped 1 tracked trainer funding/mailbox gap`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~NpcInteractionTests" --logger "trx;LogFileName=npc_interaction_shodan.trx"` -> `passed 3, skipped 1`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
...
### 2026-04-25 (Shodan quest config observation)
- Last updated: 2026-04-25
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `GossipQuestTests`, `QuestObjectiveTests`, `QuestInteractionTests`, and `StarterQuestTests` now run against `Economy.config.json` with `ECONBG1` as the quest/gossip action target, `ECONFG1` idle for topology parity, and SHODAN as director.
  - Quest location and quest-state setup moved into `LiveBotFixture` helpers; no WoWStateManager runtime change was required.
- Pass result: `Quest migration shape green; live quest group validation passed 6/6`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~GossipQuestTests|FullyQualifiedName~QuestObjectiveTests|FullyQualifiedName~QuestInteractionTests|FullyQualifiedName~StarterQuestTests" --logger "trx;LogFileName=quest_group_shodan_rerun.trx"` -> `passed (6/6)`
  - `dotnet test ... --filter "FullyQualifiedName~FishingProfessionTests.Fishing_CatchFish_BgAndFg_RatchetStagedPool" --logger "trx;LogFileName=fishing_shodan_anchor_quest_slice.trx"` -> `failed (known Ratchet anchor instability: FG loot_window_timeout/max_casts_reached)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
- Next command: `powershell -ExecutionPolicy Bypass -File .\run-tests.ps1 -CleanupRepoScopedOnly; rg -n "BotLearnSpellAsync|BotSetSkillAsync|BotAddItemAsync|BotTeleportAsync|SendGmChatCommand|ExecuteGMCommand|\\.learn|\\.additem|\\.setskill|\\.tele|\\.go|\\.send|modify money" Tests/BotRunner.Tests/LiveValidation/NpcInteractionTests.cs`

### 2026-04-25 (Shodan Trading config observation)
- Last updated: 2026-04-25
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `TradingTests` and `TradeParityTests` now run against `Economy.config.json` with `ECONFG1`/`ECONBG1` as the trade participants and SHODAN as director.
  - An experimental four-bot trade-specific roster was not kept; the existing economy topology is sufficient for the committed Shodan shape.
- Pass result: `Trading migration shape green; live trading validation passed 1 and skipped 3 tracked foreground trade action gaps`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~TradingTests|FullyQualifiedName~TradeParityTests" --logger "trx;LogFileName=trading_shodan_final.trx"` -> `1 passed, 3 skipped`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
...
### 2026-04-25 (Shodan Mail config observation)
- Last updated: 2026-04-25
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
...
### 2026-04-25 (Shodan EconomyInteraction config observation)
- Last updated: 2026-04-25
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `EconomyInteractionTests` now runs against `Economy.config.json` with `ECONFG1`/`ECONBG1` as action targets plus SHODAN.
  - Mailbox and SOAP mail-money setup moved into `LiveBotFixture` helpers; no WoWStateManager runtime change was required.
- Pass result: `EconomyInteraction migration shape green; live economy validation passed 3/3`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~EconomyInteractionTests" --logger "trx;LogFileName=economy_interaction_shodan.trx"` -> `passed (3/3)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
- Next command: `powershell -ExecutionPolicy Bypass -File .\run-tests.ps1 -CleanupRepoScopedOnly; rg -n "BotLearnSpellAsync|BotSetSkillAsync|BotAddItemAsync|BotTeleportAsync|SendGmChatCommand|ExecuteGMCommand|\\.learn|\\.additem|\\.setskill|\\.tele|\\.go|\\.send|modify money" Tests/BotRunner.Tests/LiveValidation/MailSystemTests.cs Tests/BotRunner.Tests/LiveValidation/MailParityTests.cs`

### 2026-04-25 (Shodan VendorBuySell config observation)
- Last updated: 2026-04-25
- Active task: none - this slice reused the existing Shodan economy roster and did not change WoWStateManager code or config.
- Last delta:
  - `VendorBuySellTests` now runs against `Economy.config.json` with `ECONBG1` as the BG vendor packet action target, `ECONFG1` idle for topology parity, and SHODAN as director.
  - The roster remains the base for `EconomyInteractionTests` if its action-target needs still match.
- Pass result: `VendorBuySell migration shape green; live vendor validation passed 2/2`
- Validation/tests run:
  - `dotnet test ... --filter "FullyQualifiedName~FishingPoolActivationAnalyzerTests|FullyQualifiedName~LiveBotFixtureBotChatTests|FullyQualifiedName~GatheringRouteSelectionTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (33/33)`
  - `dotnet test ... --filter "FullyQualifiedName~ActionForwardingContractTests|FullyQualifiedName~BotRunnerServiceSnapshotTests|FullyQualifiedName~BotRunnerServiceFishingDispatchTests"` -> `passed (60/60)`
  - `dotnet test ... --filter "FullyQualifiedName~VendorBuySellTests" --logger "trx;LogFileName=vendor_buy_sell_shodan.trx"` -> `passed (2/2)`
  - Repo-scoped cleanup before and after live validation -> `No repo-scoped processes to stop.`
- Files changed:
  - `Services/WoWStateManager/TASKS.md`
