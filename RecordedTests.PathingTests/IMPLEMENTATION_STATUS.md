...
- **[DllMain.cpp](../Exports/Navigation/DllMain.cpp)** - `WWOW_DATA_DIR` environment variable support
...
- **[Navigation.cpp](../Exports/Navigation/Navigation.cpp)** - `GetMmapsPath()` updated
  - Prioritizes `WWOW_DATA_DIR` for mmaps/ path resolution
...
- **[PathingAndOverlapTests.cs](../Tests/PathfindingService.Tests/PathingAndOverlapTests.cs)** - Enhanced `NavigationFixture`
  ...
  - Supports both `WWOW_DATA_DIR` and DLL-relative paths
...
3. Ensure nav data exists:
   ```powershell
   Test-Path "$env:WWOW_DATA_DIR\mmaps\*.mmtile"
   ```
