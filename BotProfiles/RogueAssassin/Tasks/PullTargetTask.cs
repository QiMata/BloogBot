...
namespace RogueAssassin.Tasks
{
    internal class PullTargetTask : BotTask, IBotTask
    {
        ...
        public void Update()
        {
            ...
            if (distanceToTarget < 30 && !ObjectManager.Player.HasBuff(Stealth) && ObjectManager.IsSpellReady(Garrote) && !ObjectManager.Player.IsInCombat)
            ...
            if (distanceToTarget < 25 && ObjectManager.IsSpellReady(Distract) && ObjectManager.IsSpellReady(Distract) && ObjectManager.Player.HasBuff(Stealth))
            {
            ...
            if (distanceToTarget < 5 && ObjectManager.Player.HasBuff(Stealth) && ObjectManager.IsSpellReady(Ambush) && DaggerEquipped && !ObjectManager.Player.IsInCombat && ObjectManager.Player.IsBehind(ObjectManager.GetTarget(ObjectManager.Player)))
            {
                ObjectManager.CastSpell(Ambush);
                return;
            }

            if (distanceToTarget < 5 && ObjectManager.Player.HasBuff(Stealth) && ObjectManager.IsSpellReady(Garrote) && !DaggerEquipped && !ObjectManager.Player.IsInCombat && ObjectManager.Player.IsBehind(ObjectManager.GetTarget(ObjectManager.Player)))
            {
                ObjectManager.CastSpell(Garrote);
                return;
            }

            if (distanceToTarget < 5 && ObjectManager.Player.HasBuff(Stealth) && ObjectManager.IsSpellReady(CheapShot) && !ObjectManager.Player.IsInCombat && !ObjectManager.Player.IsBehind(ObjectManager.GetTarget(ObjectManager.Player)))
            {
