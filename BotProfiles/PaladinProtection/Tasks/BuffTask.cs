...
namespace PaladinProtection.Tasks
{
    public class BuffTask(IBotContext botContext) : BotTask(botContext), IBotTask
    {
        ...
        public void Update()
        {
            if (!ObjectManager.IsSpellReady(BlessingOfMight) || ObjectManager.Player.HasBuff(BlessingOfMight) || ObjectManager.Player.HasBuff(BlessingOfKings) || ObjectManager.Player.HasBuff(BlessingOfSanctuary))
            {
                BotTasks.Pop();
                return;
            }
        ...
        {
            if (!ObjectManager.Player.HasBuff(name) && ObjectManager.IsSpellReady(name) && ObjectManager.Player.Mana > ObjectManager.GetManaCost(name))
            {
