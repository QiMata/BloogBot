...
public class IntegrationTestConfig
{
    ...
    #region MaNGOS Server Settings

    /// <summary>
    /// MaNGOS server installation directory.
...
public class ServiceHealthChecker(ILogger<ServiceHealthChecker>? logger = null)
{
    ...
    /// </summary>
    public Task<bool> IsMySqlAvailableAsync(IntegrationTestConfig config)
        => IsServiceAvailableAsync("127.0.0.1", config.MySqlPort, config.HealthCheckTimeoutMs);

    /// <summary>
    /// Checks if MaNGOS realmd (auth server) is available.
    /// </summary>
    public Task<bool> IsRealmdAvailableAsync(IntegrationTestConfig config)
        => IsServiceAvailableAsync(config.AuthServerIp, config.AuthServerPort, config.HealthCheckTimeoutMs);

    /// <summary>
    /// Checks if MaNGOS mangosd (world server) is available.
    /// </summary>
    public Task<bool> IsMangosdAvailableAsync(IntegrationTestConfig config)
        => IsServiceAvailableAsync("127.0.0.1", config.WorldServerPort, config.HealthCheckTimeoutMs);
}
...
public enum RequiredServices
{
    ...
    MySQL = 4,
    MangosStack = WoWServer | MySQL,
    All = WoWServer | PathfindingService | MySQL
}
