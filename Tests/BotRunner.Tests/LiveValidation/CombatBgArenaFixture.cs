...
[CollectionDefinition(CombatBgArenaCollection.Name)]
public sealed class CombatBgArenaCollection : ICollectionFixture<CombatBgArenaFixture>
{
    public const string Name = "CombatBgArenaCollection";
}

/// <summary>
/// Dedicated fresh-account fixture for the BG-only combat baseline. Two background
/// warrior accounts are staged at the Valley of Trials boar cluster so the test
/// can dispatch one shared-target melee attack per bot without any helper-owned
/// GM combat shortcuts.
/// </summary>
public sealed class CombatBgArenaFixture : CoordinatorFixtureBase
{
    ...
    public string PrimaryBgAccount => "BGONLYBG1";
    public string SecondaryBgAccount => "BGONLYBG2";

    protected override string SettingsFileName => "CombatBg.settings.json";

    protected override string FixtureLabel => "COMBAT-BG";
    ...
    protected override IReadOnlyList<CharacterSettings> BuildCharacterSettings()
        => LoadCharacterSettingsFromConfig("CombatBg.config.json");
