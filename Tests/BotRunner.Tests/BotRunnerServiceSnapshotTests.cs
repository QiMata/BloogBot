...
public class BotRunnerServiceSnapshotTests
{
    ...
    [Fact]
    public void SubscribeToMessageEvents_BuffersLearnedSpellMessages()
    {
        var eventHandler = new Mock<IWoWEventHandler>(MockBehavior.Loose);
        var objectManager = new Mock<IObjectManager>(MockBehavior.Loose);
        objectManager.SetupGet(x => x.EventHandler).Returns(eventHandler.Object);
        ...
        eventHandler.Raise(
            ...
            eventHandler.Object,
            new SpellChangedArgs(18248));

        InvokeFlushMessageBuffers(service);
        var snapshot = ReadActivitySnapshot(service);

        Assert.Contains("[SKILL] Learned spell 18248", snapshot.RecentChatMessages);
    }

    [Fact]
    public void SubscribeToMessageEvents_BuffersUnlearnedSpellMessages()
    {
        var eventHandler = new Mock<IWoWEventHandler>(MockBehavior.Loose);
        var objectManager = new Mock<IObjectManager>(MockBehavior.Loose);
        objectManager.SetupGet(x => x.EventHandler).Returns(eventHandler.Object);
        ...
        eventHandler.Raise(
            handler => handler.OnUnlearnedSpell += null!,
            eventHandler.Object,
            new SpellChangedArgs(18248));
        ...
        Assert.Contains("[SKILL] Unlearned spell 18248", snapshot.RecentChatMessages);
    }

    [Fact]
    public void SubscribeToMessageEvents_BuffersSkillUpdatedMessages()
    {
        var eventHandler = new Mock<IWoWEventHandler>(MockBehavior.Loose);
        var objectManager = new Mock<IObjectManager>(MockBehavior.Loose);
        objectManager.SetupGet(x => x.EventHandler).Returns(eventHandler.Object);
        ...
        eventHandler.Raise(
            handler => handler.OnSkillUpdated += null!,
            eventHandler.Object,
            new SkillUpdatedArgs(356, 75, 76, 150));

        InvokeFlushMessageBuffers(service);
        var snapshot = ReadActivitySnapshot(service);

        Assert.Contains("[SKILL] Skill 356 75->76/150", snapshot.RecentChatMessages);
    }

    [Fact]
    public void SubscribeToMessageEvents_BuffersItemAddedToBagMessages()
    {
        var eventHandler = new Mock<IWoWEventHandler>(MockBehavior.Loose);
        var objectManager = new Mock<IObjectManager>(MockBehavior.Loose);
        objectManager.SetupGet(x => x.EventHandler).Returns(eventHandler.Object);
        ...
        eventHandler.Raise(
            handler => handler.OnItemAddedToBag += null!,
            eventHandler.Object,
            new ItemAddedToBagArgs(0, 15, 2589, 3));

        InvokeFlushMessageBuffers(service);
        var snapshot = ReadActivitySnapshot(service);

        Assert.Contains("[UI] Item 2589 x3 -> bag 0/15", snapshot.RecentChatMessages);
    }

    [Fact]
    public void SubscribeToMessageEvents_BuffersErrorMessages()
    {
        var eventHandler = new Mock<IWoWEventHandler>(MockBehavior.Loose);
        var objectManager = new Mock<IObjectManager>(MockBehavior.Loose);
        objectManager.SetupGet(x => x.EventHandler).Returns(eventHandler.Object);
        ...
        eventHandler.Raise(
            handler => handler.OnErrorMessage += null!,
            eventHandler.Object,
            new OnUiMessageArgs("Attack failed: Not in range."));

        InvokeFlushMessageBuffers(service);
        var snapshot = ReadActivitySnapshot(service);

        Assert.Contains("[ERROR] Attack failed: Not in range.", snapshot.RecentErrors);
    }
