...
## Session Handoff
### 2026-04-25 (Dual-client/movement parity Shodan migration observation)
- Pass result: `No BotRunner production code changed; deterministic dispatch coverage stayed green and migrated dual-client/movement live validation passed overall with 10 passes and 7 tracked skips`
- Last delta:
  - `DualClientParityTests` now uses Shodan-owned shared Orgrimmar staging before BG/FG snapshot parity assertions; the legacy GM-command parity lane is a tracked skip.
  - `MovementParityTests` now dispatches only BG/FG recording, facing, and `Goto` actions after Shodan-directed route-start staging. Runtime staging/quiesce instability, insufficient live travel on one route, and redirect packet-recording edges remain explicit tracked skips.
