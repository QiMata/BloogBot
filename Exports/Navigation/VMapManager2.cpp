// VMapManager2.cpp - Enhanced with cylinder collision support
...
namespace VMAP
{
        ...
        const Cylinder& worldCylinder, const G3D::Vector3& sweepDir, float sweepDistance) const
    {
        ...
        if (instanceTree != iInstanceMapTrees.end())
        {
            ...
            // Convert all hit positions back to world coordinates
    ...
    float VMapManager2::getHeight(unsigned int pMapId, float x, float y, float z, float maxSearchDist)
    {
        ...
        float height = PhysicsConstants::INVALID_HEIGHT;
        ...
        if (instanceTree != iInstanceMapTrees.end())
        {
            ...
            if (!(height < G3D::inf()))
            {
                height = PhysicsConstants::INVALID_HEIGHT;
            }
        }
