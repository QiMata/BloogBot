namespace GameData.Core.Enums
{
    public class UpdateFields
    {
        ...
        /// <summary>
        /// 1.12.1 (build 5875) Player Update Fields — network packet field INDICES.
        /// Source: vmangos/core UpdateFields_1_12_1.cpp
        /// All values are EUnitFields.UNIT_END + offset (absolute index in update mask).
        /// </summary>
        public enum EPlayerFields : uint
        {
            PLAYER_DUEL_ARBITER = EUnitFields.UNIT_END + 0x00, // Size:2 (GUID)
            PLAYER_FLAGS = EUnitFields.UNIT_END + 0x02,
            PLAYER_GUILDID = EUnitFields.UNIT_END + 0x03,
            PLAYER_GUILDRANK = EUnitFields.UNIT_END + 0x04,
            PLAYER_BYTES = EUnitFields.UNIT_END + 0x05,
            PLAYER_BYTES_2 = EUnitFields.UNIT_END + 0x06,
            PLAYER_BYTES_3 = EUnitFields.UNIT_END + 0x07,
            PLAYER_DUEL_TEAM = EUnitFields.UNIT_END + 0x08,
            PLAYER_GUILD_TIMESTAMP = EUnitFields.UNIT_END + 0x09,

            // Quest Logs: 20 entries × 3 fields (questId, state, timer)
            PLAYER_QUEST_LOG_1_1 = EUnitFields.UNIT_END + 0x0A,
            PLAYER_QUEST_LOG_1_2 = EUnitFields.UNIT_END + 0x0B,
            PLAYER_QUEST_LOG_LAST_3 = EUnitFields.UNIT_END + 0x45,

            // Visible Items: 19 equipment slots × 12 fields (creator[2], entry[8], properties, pad)
            ...
            PLAYER_VISIBLE_ITEM_19_PAD = EUnitFields.UNIT_END + 0x129,

            // Inventory Slots: 23 equip+bag slots × 2 fields (GUID)
            PLAYER_FIELD_INV_SLOT_HEAD = EUnitFields.UNIT_END + 0x12A,

            // Backpack (Pack) Slots: 16 slots × 2 fields (GUID)
            PLAYER_FIELD_PACK_SLOT_1 = EUnitFields.UNIT_END + 0x158,
            PLAYER_FIELD_PACK_SLOT_LAST = EUnitFields.UNIT_END + 0x177,

            // Bank Slots: 24 slots × 2 fields (GUID)
            PLAYER_FIELD_BANK_SLOT_1 = EUnitFields.UNIT_END + 0x178,
            PLAYER_FIELD_BANK_SLOT_LAST = EUnitFields.UNIT_END + 0x1A7,

            // Bank Bag Slots: 6 slots × 2 fields (GUID)
            PLAYER_FIELD_BANKBAG_SLOT_1 = EUnitFields.UNIT_END + 0x1A8,
            PLAYER_FIELD_BANKBAG_SLOT_LAST = EUnitFields.UNIT_END + 0x1B3,

            // Vendor Buyback Slots: 12 slots × 2 fields (GUID)
            PLAYER_FIELD_VENDORBUYBACK_SLOT_1 = EUnitFields.UNIT_END + 0x1B4,
            PLAYER_FIELD_VENDORBUYBACK_SLOT_LAST = EUnitFields.UNIT_END + 0x1CB,

            // Keyring Slots: 32 slots × 2 fields (GUID)
            PLAYER_FIELD_KEYRING_SLOT_1 = EUnitFields.UNIT_END + 0x1CC,
            PLAYER_FIELD_KEYRING_SLOT_LAST = EUnitFields.UNIT_END + 0x20B,

            PLAYER_FARSIGHT = EUnitFields.UNIT_END + 0x20C, // Size:2 (GUID)
            PLAYER_FIELD_COMBO_TARGET = EUnitFields.UNIT_END + 0x20E, // Size:2 (GUID)

            PLAYER_XP = EUnitFields.UNIT_END + 0x210,
            PLAYER_NEXT_LEVEL_XP = EUnitFields.UNIT_END + 0x211,

            // Skill Info: 384 fields (128 skills × 3 pairs)
            ...
            // Positive/Negative stat mods: 5 fields each
            ...
            // Resistance buff mods: 7 fields positive, 7 fields negative
            ...
            // Damage done mods: 7 schools each
            ...
            // Honor system (1.12.1 vanilla)
            PLAYER_FIELD_KILLS = EUnitFields.UNIT_END + 0x426, // session kills
            ...
            PLAYER_FIELD_LIFETIME_HONORABLE_KILLS = EUnitFields.UNIT_END + 0x42B,
            PLAYER_FIELD_LIFETIME_DISHONORABLE_KILLS = EUnitFields.UNIT_END + 0x42C,
            ...
            // Combat Ratings: 20 fields
            PLAYER_FIELD_COMBAT_RATING_1 = EUnitFields.UNIT_END + 0x432,

            PLAYER_END = EUnitFields.UNIT_END + 0x446,

            // ==========================================================
            // TBC/WotLK-only fields — NOT present in 1.12.1 servers.
            // Kept for code compatibility; will never match server data.
            // ==========================================================
            PLAYER_CHOSEN_TITLE = 0xF000,
            PLAYER__FIELD_KNOWN_TITLES = 0xF001,
            PLAYER_OFFHAND_CRIT_PERCENTAGE = 0xF003,
            PLAYER_SPELL_CRIT_PERCENTAGE1 = 0xF004,
            PLAYER_FIELD_MOD_HEALING_DONE_POS = 0xF010,
            PLAYER_FIELD_MOD_TARGET_RESISTANCE = 0xF011,
            PLAYER_FIELD_ARENA_TEAM_INFO_1_1 = 0xF020,
            PLAYER_FIELD_HONOR_CURRENCY = 0xF030,
            PLAYER_FIELD_ARENA_CURRENCY = 0xF031,
            PLAYER_FIELD_MOD_MANA_REGEN = 0xF040,
            PLAYER_FIELD_MOD_MANA_REGEN_INTERRUPT = 0xF041,
            PLAYER_FIELD_MAX_LEVEL = 0xF042,
            PLAYER_FIELD_DAILY_QUESTS_1 = 0xF050,
            PLAYER_FIELD_PADDING = 0xF060,
        }
