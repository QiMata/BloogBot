...
namespace GameData.Core.Constants
{
    ...
    public static class SpellData
    {
        ...
        /// <summary>
        /// Spells that apply the mounted aura in the local VMaNGOS `spell_template`.
        /// Query source:
        /// `SELECT entry FROM spell_template WHERE effectApplyAuraName1 = 78 OR effectApplyAuraName2 = 78 OR effectApplyAuraName3 = 78 GROUP BY entry ORDER BY entry;`
        /// </summary>
        public static readonly HashSet<uint> MountSpellIds =
        [
            458, 459, 468, 470, 471, 472, 578, 579, 580, 581, 3363, 5784, 6648, 6653, 6654, 6777,
            6896, 6897, 6898, 6899, 8394, 8395, 8396, 8980, 10787, 10788, 10789, 10790, 10792,
            10793, 10795, 10796, 10798, 10799, 10800, 10801, 10802, 10803, 10804, 10873, 10969,
            13819, 15779, 15780, 15781, 16055, 16056, 16058, 16059, 16060, 16080, 16081, 16082,
            16083, 16084, 17229, 17450, 17453, 17454, 17455, 17456, 17458, 17459, 17460, 17461,
            17462, 17463, 17464, 17465, 17481, 18363, 18989, 18990, 18991, 18992, 22717, 22718,
            22719, 22720, 22721, 22722, 22723, 22724, 23161, 23214, 23219, 23220, 23221, 23222,
            23223, 23225, 23227, 23228, 23229, 23238, 23239, 23240, 23241, 23242, 23243, 23246,
            23247, 23248, 23249, 23250, 23251, 23252, 23338, 23509, 23510, 24242, 24252, 24576,
            25675, 25858, 25859, 25863, 25953, 26054, 26055, 26056, 26332, 26655, 29059, 30174,
            31700
        ];
        ...
        public static bool IsMountSpell(int spellId) => spellId > 0 && IsMountSpell((uint)spellId);

        public static bool IsMountSpell(uint spellId) => MountSpellIds.Contains(spellId);
