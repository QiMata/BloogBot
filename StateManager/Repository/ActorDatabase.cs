...
namespace StateManager.Repository
{
    public class ActorDatabase
    {
        ...
        private static readonly Dictionary<Race, Class[]> ClassesByRace = new()
        {
            ...
            {
                Race.Dwarf,
                new[] { Class.Warrior, Class.Paladin, Class.Hunter, Class.Rogue, Class.Priest }
            },
            {
                Race.NightElf,
                new[] { Class.Warrior, Class.Hunter, Class.Rogue, Class.Priest, Class.Druid }
            },
            { Race.Gnome, new[] { Class.Warrior, Class.Rogue, Class.Mage, Class.Warlock } },
            {
                Race.Orc,
                new[] { Class.Warrior, Class.Hunter, Class.Rogue, Class.Shaman, Class.Warlock }
            },
            {
                Race.Undead,
                new[] { Class.Warrior, Class.Rogue, Class.Priest, Class.Mage, Class.Warlock }
            },
        ...
        private static readonly Dictionary<Class, Skills[]> Specs = new()
        {
            ...
            {
                Class.Shaman,
                new[] { Skills.ELEMENTAL_COMBAT, Skills.ENHANCEMENT, Skills.RESTORATION }
            },
        };
        ...
        private static (float, float, float, float, float) GeneratePersonality(
            int archetype,
            Random rand
        )
        {
            ...
            {
                1 => (
                    RandomRange(rand, 0.7f, 1.0f),
                    RandomRange(rand, 0.9f, 1.0f),
                    RandomRange(rand, 0.3f, 0.6f),
                    RandomRange(rand, 0.4f, 0.7f),
                    RandomRange(rand, 0.3f, 0.6f)
                ),
                2 => (
                    RandomRange(rand, 0.6f, 0.9f),
                    RandomRange(rand, 0.7f, 0.9f),
                    RandomRange(rand, 0.8f, 1.0f),
                    RandomRange(rand, 0.3f, 0.5f),
                    RandomRange(rand, 0.5f, 0.8f)
                ),
                3 => (
                    RandomRange(rand, 0.8f, 1.0f),
                    RandomRange(rand, 0.4f, 0.6f),
                    RandomRange(rand, 0.8f, 1.0f),
                    RandomRange(rand, 0.8f, 1.0f),
                    RandomRange(rand, 0.2f, 0.4f)
                ),
                _ => (
                    RandomRange(rand, 0.4f, 0.6f),
                    RandomRange(rand, 0.7f, 0.9f),
                    RandomRange(rand, 0.1f, 0.3f),
                    RandomRange(rand, 0.6f, 0.9f),
                    RandomRange(rand, 0.3f, 0.5f)
                ),
            };
        ...
        public static string GenerateInsertScript(int actorCount)
        {
            ...
            sb.AppendLine(
                "INSERT INTO Actor (accountName, name, race, class, talent_spec, profession1, profession2, openness, conscientiousness, extroversion, agreeableness, neuroticism) VALUES"
            );

            Random rand = new();
            int currentTankCount = 0,
                currentHealerCount = 0,
                currentDpsCount = 0;
            ...
            foreach (var race in ClassesByRace.Keys)
            {
                ...
                foreach (var cls in classes)
                {
                    ...
                    foreach (var spec in specs)
                    {
                        if (
                            (
                                SpecRoleFulfillment[spec] == Role.Tank
                                && currentTankCount >= tankLimit
                            )
                            || (
                                SpecRoleFulfillment[spec] == Role.Healer
                                && currentHealerCount >= healerLimit
                            )
                            || (
                        ...
                        var (
                            ...
                            agreeableness,
                            neuroticism
                        ) = GeneratePersonality(rand.Next(1, 4), rand);
                        ...
                        sb.AppendLine(
                            $"('{accountName}', '{name}', '{race}', '{cls}', '{spec}', '{professionPair.Item1}', '{professionPair.Item2}', {openness:F2}, {conscientiousness:F2}, {extroversion:F2}, {agreeableness:F2}, {neuroticism:F2})"
                        );

                        // Track count for role distribution
                        if (SpecRoleFulfillment[spec] == Role.Tank)
                            currentTankCount++;
                        else if (SpecRoleFulfillment[spec] == Role.Healer)
                            currentHealerCount++;
                        else if (SpecRoleFulfillment[spec] == Role.DPS)
                            currentDpsCount++;

                        if (
                            currentTankCount >= tankLimit
                            && currentHealerCount >= healerLimit
                            && currentDpsCount >= dpsLimit
                        )
                            break;
                    }
                    if (
