...
- Fresh transport-helper disassembly now narrows the transform math that the later `0x631E70` contact rewrite uses:
  ...
  - `0x63214C..0x632270` first fast-exits when `transportGuid == 0`, then fast-exits again when the cached contact count at `0xC4E530` is zero. Otherwise it walks the cached `0x34`-byte contact buffer at `0xC4E534`: transform the three points at `+0x10/+0x1C/+0x28`, rotate the normal at `+0x00/+0x04/+0x08`, and recompute `planeD` at `+0x0C` from the transformed first point.
