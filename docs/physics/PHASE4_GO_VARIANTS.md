...
|---|---|---|---|
| ~~BRM south-face stall~~ | ~~`displayId=4652`~~ | n/a | **Reversed 2026-05-09.** displayId=4652 is a 1-scale Golden Sansam herb, pool-driven (max_limit=2). DB survey of 120y x 120y around the stall coord finds zero structural GOs. The 12y phantom navmesh at the stall is a bake-side `rcFilter` artifact, NOT GO-derived collision. See `project_pfs_overhaul_006_brm_gameobject_4652.md` (REVERSED). BRM unblock is tracked separately. |
...
| Static-world structural GOs (campfires, banner posts, fences, signposts) | per `gameobject` table, not pool-driven | always | Bots with sub-cm capsule clearance through narrow gates/fences could walk through them today; with the variant pool ingested they get proper collision. Pre-Phase-4 these were ONLY in transports; the Phase 4 base variant covers all of them. |
...
| Phase | Scope | Exit |
|---|---|---|
| **A** | GameObjectExporter `--variant base` filters `event_id IS NULL AND pool_template IS NULL` spawns. SceneCacheBuilder pre-bakes per-tile `.base.scenecache` binary. DynamicObjectRegistry gains a `LoadVariantSceneCache(mapId, variantId, path)` + per-`(mapId, variantId)` triangle pool. Runtime loads variants automatically from `<dataRoot>/scene-cache/`. Probe diagnostic exposes a `dynamic=N` ground-Z field showing variant-pool contribution. **No protobuf change yet** — single-variant default, MmapGen polymesh injection deferred to Phase A2. | **Revised 2026-05-09 (BRM premise reversed):** Probe at a confirmed structural GO XY reports `dynamic != -200000`. OG zeppelin canonical recipe unchanged. Unit + physics + Layer-B baselines hold. The original BRM exit-criterion is dropped; BRM unblock is now tracked under a separate `tools/MmapGen --debug-heightfield` workstream, since the 12y phantom polygon is an `rcFilter` pipeline artifact, not GO collision. |
