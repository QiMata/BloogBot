...
- Completed: Advanced **P2.2 ACK format parity** again. Extended the reusable FG GM-command probe harness with prep commands, captured live `CMSG_FORCE_MOVE_ROOT_ACK`, `CMSG_FORCE_MOVE_UNROOT_ACK`, `CMSG_MOVE_WATER_WALK_ACK`, `CMSG_MOVE_HOVER_ACK`, and `CMSG_MOVE_FEATHER_FALL_ACK` bytes from `WoW.exe` `NetClient::Send` (`0x005379A0`), and closed a newly verified byte-format gap: WoW.exe appends a trailing float toggle marker to water-walk / hover / feather-fall ACKs (`1.0f` apply, `0.0f` clear).
- Remaining gap: 3 wired ACK opcodes still need golden-corpus entries and parity tests (`CMSG_FORCE_SWIM_BACK_SPEED_CHANGE_ACK`, `CMSG_FORCE_TURN_RATE_CHANGE_ACK`, `CMSG_MOVE_KNOCK_BACK_ACK`). The unwired deferred items remain `MSG_MOVE_SET_RAW_POSITION_ACK` and `CMSG_MOVE_FLIGHT_ACK`.
- Commands run:
  ...
  - `docker exec maria-db mariadb -uroot -proot mangos -e "SELECT entry, spellName1, effectApplyAuraName1, effectApplyAuraName2, effectApplyAuraName3 FROM spell_template WHERE entry IN (1706,339,130,546,11319);"` -> confirmed the live self-aura probe IDs used for capture (`1706` levitate family, `339` root).
  - `$env:WWOW_ENABLE_RECORDING_ARTIFACTS='1'; $env:WWOW_CAPTURE_ACK_CORPUS='1'; $env:WWOW_ACK_CORPUS_OUTPUT='E:/repos/Westworld of Warcraft/Tests/WoWSharpClient.Tests/Fixtures/ack_golden_corpus'; $env:WWOW_REPO_ROOT='E:/repos/Westworld of Warcraft'; $env:WWOW_DATA_DIR='D:/MaNGOS/data'; $env:WWOW_ACK_CAPTURE_PREP_GM_COMMANDS='.targetself'; $env:WWOW_ACK_CAPTURE_GM_COMMAND='.aura 1706'; $env:WWOW_ACK_CAPTURE_RESET_GM_COMMAND='.unaura 1706'; dotnet test Tests/BotRunner.Tests/BotRunner.Tests.csproj --configuration Release --no-build --no-restore -m:1 -p:UseSharedCompilation=false --filter "FullyQualifiedName~AckCaptureTests.Foreground_GmCommand_CapturesConfiguredAckCorpusWhenEnabled" --logger "console;verbosity=minimal"` -> passed (`1/1`); produced representative `CMSG_MOVE_WATER_WALK_ACK`, `CMSG_MOVE_HOVER_ACK`, and `CMSG_MOVE_FEATHER_FALL_ACK` fixtures.
  - `$env:WWOW_ENABLE_RECORDING_ARTIFACTS='1'; $env:WWOW_CAPTURE_ACK_CORPUS='1'; $env:WWOW_ACK_CORPUS_OUTPUT='E:/repos/Westworld of Warcraft/Tests/WoWSharpClient.Tests/Fixtures/ack_golden_corpus'; $env:WWOW_REPO_ROOT='E:/repos/Westworld of Warcraft'; $env:WWOW_DATA_DIR='D:/MaNGOS/data'; $env:WWOW_ACK_CAPTURE_PREP_GM_COMMANDS='.targetself'; $env:WWOW_ACK_CAPTURE_GM_COMMAND='.aura 339'; $env:WWOW_ACK_CAPTURE_RESET_GM_COMMAND='.unaura 339'; dotnet test Tests/BotRunner.Tests/BotRunner.Tests.csproj --configuration Release --no-build --no-restore -m:1 -p:UseSharedCompilation=false --filter "FullyQualifiedName~AckCaptureTests.Foreground_GmCommand_CapturesConfiguredAckCorpusWhenEnabled" --logger "console;verbosity=minimal"` -> passed (`1/1`); produced representative `CMSG_FORCE_MOVE_ROOT_ACK` and `CMSG_FORCE_MOVE_UNROOT_ACK` fixtures.
...
- Files changed: `Exports/WoWSharpClient/Parsers/MovementPacketHandler.cs`, `Exports/WoWSharpClient/WoWSharpObjectManager.Movement.cs`, `Services/ForegroundBotRunner/Diagnostics/ForegroundAckCorpusRecorder.cs`, `Tests/BotRunner.Tests/LiveValidation/AckCaptureTests.cs`, `Tests/WoWSharpClient.Tests/ObjectManagerWorldSessionTests.cs`, `Tests/WoWSharpClient.Tests/Parity/AckBinaryParityTests.cs`, and new fixtures under `Tests/WoWSharpClient.Tests/Fixtures/ack_golden_corpus/{CMSG_FORCE_MOVE_ROOT_ACK,CMSG_FORCE_MOVE_UNROOT_ACK,CMSG_MOVE_WATER_WALK_ACK,CMSG_MOVE_HOVER_ACK,CMSG_MOVE_FEATHER_FALL_ACK,MSG_MOVE_TELEPORT_ACK}`.
- Next command: `rg -n "CMSG_FORCE_SWIM_BACK_SPEED_CHANGE_ACK|CMSG_FORCE_TURN_RATE_CHANGE_ACK|CMSG_MOVE_KNOCK_BACK_ACK|knockback|turn rate|bwalk|swim back" docs/physics Exports/WoWSharpClient Tests Services -g '!**/bin/**' -g '!**/obj/**'`.
