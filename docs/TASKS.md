...
| 22.31 | **Configurable spec test** — `Tests/BotRunner.Tests/Progression/ConfigurableSpecTests.cs`. Create CharacterSettings with `BuildConfig.SpecName = "WarriorFury"`. Assert: bot loads Fury profile, not Arms (default). Change to "WarriorProtection" → loads Prot profile. | Open |
...
| 21.3 | **Create `TravelOptions` record** — File: `Exports/BotRunner/Tasks/Travel/TravelOptions.cs`. Fields: `bool AllowHearthstone`, `bool AllowClassTeleport`, `bool AllowFlightPath`, `Faction PlayerFaction`, `IReadOnlyCollection<uint> DiscoveredFlightNodes`, `uint? HearthstoneBindMapId`, `Position? HearthstoneBindPosition`, `float HearthstoneCooldownRemainingSec`. Used by TravelTask to choose optimal route. | Open |
...
| 21.22 | **Named location resolver** — File: `Exports/BotRunner/Travel/LocationResolver.cs`. Static dictionary mapping friendly names to (mapId, position): "Orgrimmar" → (1, 1676, -4315, 61), "Stormwind" → (0, -8913, 554, 94), "Ironforge" → (0, -4981, -881, 502), all capital cities + major quest hubs + dungeon entrances. Used by TravelObjective when `targetLocationName` is set instead of raw coordinates. Load additional locations from MaNGOS `game_tele` table (read-only query) at startup. | Open |
...
| 9.21 | **Replace `Dictionary<string, ...> + lock`** — Use `ConcurrentDictionary` everywhere. Remove `_managedServicesLock`. Use `ConcurrentDictionary.AddOrUpdate()` for atomic operations. Partition coordination scan: combat coordinator only scans bots in same zone/area. | Open |
