...
Last refresh: 2026-05-13 (FlameCrest UBRS stall PHASE 1 LANDED — FG-rendering reconnaissance harness + property tests committed as 54cb9b9d "Establish BRM ascent rendering ground truth + property tests". Phase 1 inverted the bench-green/live-regress pattern by establishing FG-client rendering as the goalpost. 44 PNGs + 11 JSONs captured at 11 coords on the FC->BRM route; RECON_SUMMARY.md at tmp/test-runtime/screenshots/brm-ascent-recon/ documents all observations. 14 property tests in BrmAscentRenderingExpectations.cs: 10 pass on current bake, 4 fail intentionally as the Phase 2 iteration gates (3 portal-corridor terminus mismatches + SmoothPath_FcToUbrsPortal_NoUnreasonableZJump with 58 violations). Diagnostic dump (DumpZJumpZone) localized the headline failure: smooth-path WP[663]->WP[664] 27y vertical jump is on a SINGLE polygon 0x0001000014F002AC (Detour tile-bits 0x14F, area=7 MAGMA, flags=0x0002 NAV_MAGMA only) that spans 36+y vertically (z=90 to z=126 surface). Phase 2 candidate surfaces: E targeted poly cull (NavMeshTileEditor flags=0 on 0x14F002AC) or C per-tile cs/maxSimplificationError tightening for tile 0x14F. Phase 2 NOT attempted this session — both surfaces require fresh attempt cycles + ~30-40 min live FG validation per try; deferred to next session with the recon as the baseline.).
