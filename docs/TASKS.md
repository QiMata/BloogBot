...
- Completed: Started the **P2.4 ObjectManager state mutation parity** pass. Added `docs/physics/0x4651A0_disasm.txt`, `docs/physics/smsg_update_object_handler.md`, and `docs/physics/cgobject_layout.md`, and fixed the first binary-backed object-update gap: BG no longer treats `UPDATETYPE_NEAR_OBJECTS` (`5`) as unhandled and now removes stale cached objects for that GUID list before the follow-up create blocks arrive.
- Binary-backed note: the fresh `0x4651A0` capture shows a three-stage `SMSG_UPDATE_OBJECT` flow: read `blockCount` + lead byte, run `0x466010` prepass(es), then dispatch blocks through `0x465330` (partial), `0x465EA0` (movement), `0x465C50` (create/create2), and `0x465FD0` (type `4` and `5` GUID lists). The key new fix is anchored to `0x467230 -> 0x4644F0`, which proves the type-`5` prepass actively removes stale cached objects instead of ignoring the GUID list.
...
- Files changed: `docs/TASKS.md`, `docs/physics/README.md`, `docs/physics/0x4651A0_disasm.txt`, `docs/physics/smsg_update_object_handler.md`, `docs/physics/cgobject_layout.md`, `Exports/WoWSharpClient/Handlers/ObjectUpdateHandler.cs`, `Exports/WoWSharpClient/TASKS.md`, `Tests/WoWSharpClient.Tests/Handlers/SMSG_UPDATE_OBJECT_Tests.cs`, `Tests/WoWSharpClient.Tests/TASKS.md`, and `memory/wow_exe_physics_decompilation.md`.
- Next command: `rg -n "466590|466320|466A20|ObjectUpdateMutationOrderTests|NEAR_OBJECTS" docs/physics Exports/WoWSharpClient Tests/WoWSharpClient.Tests -g '!**/bin/**' -g '!**/obj/**'`
