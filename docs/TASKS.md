...
Last refresh: 2026-05-13 (nav-summary accelerator scaffold; S1.3 mesh blocker still tracked separately).
...
- 2026-05-12 OG tower drill-down:
  ...
  - 2026-05-13 focused runtime regen rewrote `D:\MaNGOS\data\mmaps\0012940.mmtile`, `0004533.mmtile`, and `0004634.mmtile`; backups live under the stable `og-zeppelin/latest/backup/` and `brd/latest/backup/` folders. OG remains clean in the top-deck crop (`171` polygons; worst `zRange=1.20y`; largest area `39.750`). BRD/BRM comparison bundle now lives under `tmp/test-runtime/visualization/pathfinding/brd/latest/`: `brd_approach_mmap_crop_polys.csv` now has `608` polygons and peaks at `zRange=16.70y`; `brm_south_trap_mmap_crop_polys.csv` now has `1199` polygons and peaks at `zRange=39.70y`, still supporting a bake/filter/WMO-connectivity problem rather than a BotRunner route-execution-only problem. `analysis/suspicious_poly_brd_approach.obj` and `analysis/suspicious_poly_brm_south_trap.obj` isolate the current worst polygons.
  - Validation after regen: focused `NavDataAudit` passed for OG tile `1 40,29` with the fresh runtime log proving `baked=16, candidates=16`; focused `NavDataAudit` passed Detour/header capsule checks for BRD/BRM map-0 tiles but failed GO-bake assertions because those audited tiles have no modeled GO spawn origins and only fallback span boxes. `MmapMeshQualityTests.OrgrimmarZeppelinTopRampDeck_HasNoLargeBridgePolygons` passed `1/1` against `D:\MaNGOS\data`.
  - 2026-05-13 top-deck missing-piece drill-down:
    ...
    - The corrected top-deck source crop contains `1930` VMAP triangles and no ADT/GO triangles, so the visible connector model is present in VMAP source geometry.
    - `analysis/top_deck_surface_coverage.md` shows the `z=52.5..54.8` VMAP source band has about `679 yd^2` of upward surface while the current in-tree generated/runtime Detour crop has about `353 yd^2`. `mmapgen_detailmesh.obj` and the final Detour crop match, so the loss happens before Detour serialization/readback.
    - Stage CSV evidence points at Recast erosion: non-null compact spans in the focused crop drop from `190731` at `buildCHF` to `90823` after `rcErodeWalkableArea`.
    - Radius probe under `analysis/radius-probes/` / `mmap/radius-probes/` adds a tile-local `agentRadius=0.3064`; the top-deck crop grows from `171` to `248` polygons and route-start-reachable deck polygons from `123` to `163`. This proves the missing connector pieces are removed by largest-capsule erosion, not missing source extraction or static-GO bake.
    - Do not promote the small-radius probe directly as the fix. Next design choice is either a separate movement-footprint erosion radius while preserving the Detour/header agent capsule, or per-race/capsule mmap sets. Validate with FG screenshot/settle evidence before changing the production bake.
...
**Next pickup options:**
1. Open the refreshed OBJ/CSV artifacts under `tmp/test-runtime/visualization/pathfinding/og-zeppelin/latest/` and `tmp/test-runtime/visualization/pathfinding/brd/latest/`; for BRD/BRM, inspect the suspicious polygon OBJs and then continue MmapGen/Recast filter/WMO connectivity work.
2. For OG top-deck missing pieces, prototype a source-side bake knob that separates `agentRadius` metadata from `rcErodeWalkableArea` erosion radius, then validate against FG screenshot/settle evidence before promoting it.
