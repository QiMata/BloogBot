...
| # | Task | Owner | Status |
|---|------|-------|--------|
| 7.1 | **Log planned vs executed path.** Record `PathfindingService` waypoints AND actual bot positions at each movement tick during ghost corpse run. | `Exports/Navigation/`, `Services/PathfindingService/`, `Exports/BotRunner/Movement/NavigationPath.cs` | In progress - `NavigationPath.TraceSnapshot` now captures requested start/end, raw service waypoints, runtime waypoints, plan version, explicit replan reason, and per-tick execution samples. The next slice is surfacing that trace in corpse-run / fishing diagnostics so live failures name the exact divergence point. |
...
- **Last updated:** 2026-03-12 (session 74)
- **Current work:** P7.1/P7.4/P7.6 are still in progress. BotRunner now has planned-vs-executed route capture in `NavigationPath`, so the next slice is consuming that trace inside corpse-run and fishing diagnostics to pinpoint the exact live divergence point before touching more native route shaping.
- **Completed this session:**
  ...
  2. **Pinned deterministic trace attribution:** [NavigationPathTests.cs](E:/repos/Westworld%20of%20Warcraft/Tests/BotRunner.Tests/Movement/NavigationPathTests.cs) now proves short-route trace capture, stall-driven replan attribution, and direct-fallback attribution.
  3. **Revalidated the BotRunner slice:** the BotRunner project and test project both rebuild cleanly, and the focused `NavigationPathTests` filter stays green on the traced pathing contract.
...
- **Next:** `Get-Content Exports/BotRunner/Tasks/RetrieveCorpseTask.cs | Select-Object -Skip 180 -First 220`
