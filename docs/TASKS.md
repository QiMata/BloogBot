...
Last refresh: 2026-05-13 (BRD/BRM steep-slope walkable-band closure; mesh-quality regression added; corridor coverage Flame Crest -> {BRD,LBRS,UBRS,BWL} now 4/4 endpoints).
...
- 2026-05-12 OG tower drill-down:
  ...
  - 2026-05-13 walkable-slope band closure (this iteration): the Flame Crest stall crop on the new tiles still contained 16 NAV_STEEP_SLOPES-flagged walkable polygons inside a 25y radius of `(-7519,-2100,130)`, peak `zRange=46.8y` on `polyIndex=7926` (area=3 AREA_STEEP_SLOPE, flags=0x11). These were 52-75 degree BRM rock-face slopes; `PathFinder::createFilter` was including them via NAV_GROUND with a 10x area cost so Detour smooth paths string-pulled across them and FG physics stalled. Per-tile config now sets `walkableSlopeAngle=52` and `walkableSlopeAngleVMaps=52` for tiles `3345`, `3446`, `3546`, aligning the bake with the hardcoded 52 degree player limit so the AREA_STEEP_SLOPE band closes (slopes >52 degrees become AREA_NONE and are not baked). New `MmapMeshQualityTests.FlameCrestStall_HasNoTallSteepSlopeWallsNearStall` and `FlameCrestStall_HasNoUnreasonableGroundBridgePolygons` regressions fingerprint the previous failure mode; both pass on the regenerated tiles. OG mesh-quality regressions remain green (map-0-only change). After regen `BrmDungeonRouteDiagnostic.Audit_BrmDungeonEndpoints_ResolveAndCorridor` reports every dungeon corridor ending at the target polygon (Flame Crest -> {BRD,LBRS,UBRS,BWL} = 4/4 ends-at-target, previously 1/4); TRX `tmp/test-runtime/results-pathfinding/brm_dungeon_route_diagnostic_after_walkable_slope_52.trx`. New `BrmDungeonRouteDiagnostic.Dump_UbrsRoute_FlameCrestStallWaypoints` reports the smooth path Flame Crest -> UBRS no longer enters a 25y XY radius around the stall coord (closest waypoint idx=32 at 33.73y, all Ground polys); TRX `tmp/test-runtime/results-pathfinding/brm_flamecrest_after_walkable_slope_52.trx`. Live `LongPathingTests.FlameCrestToBrmDungeonEntrance` rerun was not run in this session because of harness/client timing; the next session should run it against the promoted runtime tiles. Backups under `tmp/test-runtime/visualization/pathfinding/brd/latest/backup/0004{533,634,635}.before_walkable_slope_angle_52.mmtile`. Tile sizes shrunk ~40-50% (0004635: 1.71 MB -> 1.03 MB; 0004533: 2.72 MB -> 1.39 MB; 0004634: 3.02 MB -> 1.85 MB), consistent with the removed steep-slope polygons. `tools/scripts/export-pathfinding-reference.ps1::Invoke-MmapVisualize` had a PowerShell 5.1 NativeCommandError bug when MmapVisualize wrote its informational `# tile=(...)` banner to stderr; scoped `$ErrorActionPreference='Continue'` around the dotnet call in that function so the exporter can be re-run without spurious failures.
