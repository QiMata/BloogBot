...
- Completed: Started **P2.2 ACK format parity**. Added foreground ACK-corpus capture plumbing on the `WoW.exe` `NetClient::Send` path (`0x005379A0`), persisted live fixture JSON under `Tests/WoWSharpClient.Tests/Fixtures/ack_golden_corpus`, captured a real `MSG_MOVE_TELEPORT_ACK`, and added the first `[Trait("Category", "AckParity")]` parity test proving `MovementPacketHandler.BuildMoveTeleportAckPayload(...)` matches the captured bytes.
- Evidence gap: the same live FG run produced `MSG_MOVE_TELEPORT_ACK` but not `MSG_MOVE_WORLDPORT_ACK`. Current evidence points to the recorder subscription happening after world entry, so the initial worldport ACK is likely sent before `ForegroundAckCorpusRecorder` starts.
- Commands run:
  ...
  - `tasklist /FI "IMAGENAME eq WoW.exe" /FO LIST` -> no running `WoW.exe` before focused validation and `AckParity` test execution.
...
- Files changed: `Services/ForegroundBotRunner/Mem/Hooks/PacketLogger.cs`, `Services/ForegroundBotRunner/Diagnostics/ForegroundAckCorpusRecorder.cs`, `Services/ForegroundBotRunner/ForegroundBotWorker.cs`, `Tests/ForegroundBotRunner.Tests/ForegroundAckCorpusRecorderTests.cs`, `Tests/WoWSharpClient.Tests/Parity/AckBinaryParityTests.cs`, `Tests/WoWSharpClient.Tests/Fixtures/ack_golden_corpus/MSG_MOVE_TELEPORT_ACK/20260417_155147_750_0000.json`, plus the earlier P2.1 decompilation docs already committed under `docs/physics/`.
- Next command: `rg -n "InitializeObjectManager|PacketLogger|ForegroundAckCorpusRecorder|MSG_MOVE_WORLDPORT_ACK" Services/ForegroundBotRunner Tests -g '!**/bin/**' -g '!**/obj/**'`.
