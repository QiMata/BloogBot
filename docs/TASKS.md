...
- Completed: Advanced **P2.2 ACK format parity**. Added an FG live cross-map capture harness, captured real `MSG_MOVE_WORLDPORT_ACK` bytes from `WoW.exe` `NetClient::Send` (`0x005379A0`), and extended `AckBinaryParityTests` so live-backed parity now covers both `MSG_MOVE_TELEPORT_ACK` and `MSG_MOVE_WORLDPORT_ACK`.
- Remaining gap: 12 wired ACK opcodes still need golden-corpus entries and parity tests (`CMSG_FORCE_*_ACK`, move root/unroot, water walk, hover, feather fall, knockback, raw-position ACK, and flight ACK).
- Commands run:
...
- Files changed: `Tests/BotRunner.Tests/LiveValidation/AckCaptureTests.cs`, `Tests/WoWSharpClient.Tests/Parity/AckBinaryParityTests.cs`, `Tests/WoWSharpClient.Tests/Fixtures/ack_golden_corpus/MSG_MOVE_WORLDPORT_ACK/20260417_161214_670_0001.json`, `Tests/WoWSharpClient.Tests/Fixtures/ack_golden_corpus/MSG_MOVE_WORLDPORT_ACK/20260417_161217_932_0002.json`, and the earlier ACK-corpus plumbing under `Services/ForegroundBotRunner/`.
- Next command: `rg -n "CMSG_FORCE_.*ACK|MSG_MOVE_SET_RAW_POSITION_ACK|CMSG_MOVE_FLIGHT_ACK" Exports/WoWSharpClient Tests Services -g '!**/bin/**' -g '!**/obj/**'`.
