...
| 7.6 | **Overlay-aware path validation and repair.** Validate native mmap routes against live object overlays, then reform blocked segments into usable local detours when possible. | `Services/PathfindingService/Repository/Navigation.cs`, `Exports/Navigation/PathFinder.cpp`, `Exports/Navigation/SceneQuery.cpp` | In progress - the service now validates returned segments against mounted dynamic-object overlays, retries alternate native mode, performs a bounded local detour search, and has a native `ValidateWalkableSegment` bridge plus result-mapping coverage ready for short-segment rollout; the new native check is intentionally gated behind `WWOW_ENABLE_NATIVE_SEGMENT_VALIDATION` until support-surface probes stop false-negative blocking on long Orgrimmar/corpse-run routes |
...
- **Current work:** P7.6 is still in progress. The native `NAV-OBJ-002` bridge now exists (`ValidateWalkableSegment`) and the service can map native walkability failures into explicit result codes, but default rollout remains gated behind `WWOW_ENABLE_NATIVE_SEGMENT_VALIDATION` until the support-surface probe stops false-negative blocking on long Orgrimmar/corpse-run segments.
- **Completed this session:**
  1. **Shipped the native walkability bridge:** [DllMain.cpp](E:/repos/Westworld%20of%20Warcraft/Exports/Navigation/DllMain.cpp) now exports `ValidateWalkableSegment`, which uses the native capsule sweep plus support/overlap checks to classify `clear`, `blocked_geometry`, `missing_support`, `step_up_too_high`, and `step_down_too_far`.
  2. **Added managed result mapping without regressing default routes:** [Navigation.cs](E:/repos/Westworld%20of%20Warcraft/Services/PathfindingService/Repository/Navigation.cs) can now surface `repaired_segment_validation`, `blocked_by_capsule_validation`, `blocked_by_step_up_limit`, and `blocked_by_step_down_limit`, while the default service path keeps the new native validator behind `WWOW_ENABLE_NATIVE_SEGMENT_VALIDATION`.
  3. **Added deterministic coverage on both sides:** [NavigationOverlayAwarePathTests.cs](E:/repos/Westworld%20of%20Warcraft/Tests/PathfindingService.Tests/NavigationOverlayAwarePathTests.cs) now covers non-dynamic repair/result mapping, and new [SegmentWalkabilityTests.cs](E:/repos/Westworld%20of%20Warcraft/Tests/Navigation.Physics.Tests/SegmentWalkabilityTests.cs) covers the native export directly.
- **Commands run + outcomes:**
  ...
  2. `dotnet build Tests/PathfindingService.Tests/PathfindingService.Tests.csproj --configuration Release --no-restore -m:1` -> succeeded.
  3. `dotnet build Tests/Navigation.Physics.Tests/Navigation.Physics.Tests.csproj --configuration Release --no-restore -m:1` -> succeeded.
  4. `dotnet test Tests/PathfindingService.Tests/PathfindingService.Tests.csproj --configuration Release --no-build --no-restore --settings Tests/PathfindingService.Tests/test.runsettings --filter "FullyQualifiedName~NavigationOverlayAwarePathTests|FullyQualifiedName~PathfindingTests" --logger "console;verbosity=minimal"` -> `9 passed`.
  5. `dotnet test Tests/Navigation.Physics.Tests/Navigation.Physics.Tests.csproj --configuration Release --no-build --no-restore --settings Tests/Navigation.Physics.Tests/test.runsettings --filter "FullyQualifiedName~SegmentWalkabilityTests" --logger "console;verbosity=minimal"` -> `2 passed`.
  6. `dotnet test Tests/PathfindingService.Tests/PathfindingService.Tests.csproj --configuration Release --no-build --no-restore --settings Tests/PathfindingService.Tests/test.runsettings --logger "console;verbosity=minimal"` -> `34 passed`.
- **Next:** `Get-Content Exports/Navigation/SceneQuery.cpp | Select-Object -Skip 520 -First 260`
