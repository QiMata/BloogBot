...
- **Last updated:** 2026-03-12 (session 79)
- **Current work:** P0A.6 gathering slice is now task-owned and pool-aware for mining, while P7.1/P7.4/P7.6 remain in progress for corpse/fishing pathing. The fixture/login `ValleyOfTrials` suspicion was rechecked and no fixture-level Valley teleport was found; only the mining test stages there explicitly.
- **Completed this session:**
  1. **Widened the Valley mining query to include full spawn-pool context:** [LiveBotFixture.cs](E:/repos/Westworld%20of%20Warcraft/Tests/BotRunner.Tests/LiveValidation/LiveBotFixture.cs) now joins `pool_gameobject` and `pool_template` so nearby-node queries return pool metadata instead of silently flattening pooled spawns.
  2. **Removed the effective 6-node truncation from Valley mining selection:** [GatheringRouteSelection.cs](E:/repos/Westworld%20of%20Warcraft/Tests/BotRunner.Tests/LiveValidation/GatheringRouteSelection.cs), [GatheringRouteSelectionTests.cs](E:/repos/Westworld%20of%20Warcraft/Tests/BotRunner.Tests/LiveValidation/GatheringRouteSelectionTests.cs), and [GatheringProfessionTests.cs](E:/repos/Westworld%20of%20Warcraft/Tests/BotRunner.Tests/LiveValidation/GatheringProfessionTests.cs) now keep pool metadata, default to the full nearby set, and log which Valley pools were loaded.
  3. **Verified the live Valley candidate count against the running world DB and the mining test:** the focused rerun now reports `7` nearby Valley copper-route candidates, all from pool `1024`, and skips only because all natural candidates were on respawn at the time of the run.
- **Commands run + outcomes:**
  1. `C:\xampp\mysql\bin\mysql.exe -h 127.0.0.1 -u root -proot -D mangos -e "SELECT g.guid, g.id, g.map, ROUND(g.position_x,1) AS x, ROUND(g.position_y,1) AS y, ROUND(g.position_z,1) AS z, ROUND(SQRT(POW(g.position_x - (-800.0), 2) + POW(g.position_y - (-4500.0), 2)), 1) AS distance2D, pg.pool_entry, pt.max_limit, pt.description FROM gameobject g LEFT JOIN pool_gameobject pg ON pg.guid = g.guid LEFT JOIN pool_template pt ON pt.entry = pg.pool_entry WHERE g.id = 1731 AND g.map = 1 AND SQRT(POW(g.position_x - (-800.0), 2) + POW(g.position_y - (-4500.0), 2)) <= 260.0 ORDER BY distance2D, g.guid;"` -> `7` Valley copper candidates found; all in pool `1024` (`Copper Veins - Durotar (Master Pool)`).
  2. `dotnet build Tests/BotRunner.Tests/BotRunner.Tests.csproj --configuration Release --no-restore -p:UseSharedCompilation=false` -> `succeeded`.
...
- **Next:** `Get-Content Tests/BotRunner.Tests/LiveValidation/GatheringProfessionTests.cs | Select-Object -Skip 240 -First 220`
