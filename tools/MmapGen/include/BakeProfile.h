...
// from TileWorker.cpp or MapBuilder.cpp. Iter 24 wires
// MakeBakeProfile(kTaurenM, false) into TileWorker.cpp::from_json(rcConfig) for
// cs/ch, removes the unconditional `config.ch = 0.1f` override in
...
// AND auto-derives ch=cs/2 per-tile when only a cs override is specified in
// per-tile config.json (so tiles like 4029 with cs=0.1 override + no ch
// override get matching ch=0.05 instead of mismatched ch=0.2562 from default).
